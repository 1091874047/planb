...
//取基本信息
func SysGetInfo() string {
	return fmt.Sprintf("开机时长:%s\n当前用户:%s\n当前系统:%s\n系统版本:%s\nBios:%s\nMotherboard:\t%s\nCPU:\t%s\nMemory:\t%s\nDisk:\t%v\nInterfaces:\t%v\n", SysGetStartTime(), SysGetUserName(), runtime.GOOS, SysGetVer(), SysGetBiosInfo(), SysGetMotherboardInfo(), SysGetCpuInfo(), SysGetMemory(), SysGetDiskInfo(), SysGetIntfs())
}

//取开机时间
func SysGetStartTime() string {
	kernel := syscall.NewLazyDLL("Kernel32.dll")
...
//取硬盘信息
func SysGetDiskInfo() (infos []diskusage) {
	kernel := syscall.NewLazyDLL("Kernel32.dll")
...
//取主板信息
func SysGetMotherboardInfo() string {
	var s []struct {
		Product string
	}
