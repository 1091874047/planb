...
func StrGetRight(orig string, findStr string) string {
	n := strings.Index(orig, findStr) + len(findStr)
	if n == -1 {
		return ""
	}
	return string([]byte(orig)[n:])
...
/**
批量取文本中间（被查找的文本 string， 前面文本 string， 后面文本 string） 返回取到的文本切片[]string
取所有符合条件的文本并返回
*/
func StrGetSubBatch(orig string, first string, last string) []string {
	res := make([]string, 0)
	for {
		temp := StrGetSub(orig, first, last)
		if temp == "" {
			break
		}
		res = append(res, temp)
		orig = strings.Replace(orig, first+temp+last, "", 1)
	}
	return res
}
...
func StrGetNum(orig string) string {
	...
	for _, s := range reg.FindAllString(orig, -1) {
		res += s
	}
	return res
}
